...
export function DocsListing({ allDocs, allTags }: DocsListingProps) {
      ...
      <aside className={`filters-sidebar ${isMobileFiltersOpen ? 'open' : ''}`}>
        <div style={{ position: "sticky", top: "100px", display: "flex", flexDirection: "column", gap: "32px" }}>
          ...
          <div>
            ...
            <SearchInput value={searchQuery} onChange={setSearchQuery} placeholder="Search docs..." />
          </div>
      ...
      <div className="articles-list">
        {filteredDocs.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {filteredDocs.map((doc, i, arr) => (
              <Link
                key={doc.slug}
                href={`/docs/${doc.slug.split("/").pop()}`}
              ...
              >
                ...
                >
                  ...
                  <h2 style={{ fontFamily: "var(--serif)", fontSize: "1.3rem", fontWeight: 400, color: "var(--text)", lineHeight: 1.3, margin: 0 }}>
                    {doc.title}
                  </h2>
