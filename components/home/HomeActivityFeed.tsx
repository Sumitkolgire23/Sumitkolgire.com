...
export function HomeActivityFeed({
  ...
  // 2. Fetch contributions data dynamically on selectedYear change
  useEffect(() => {
    async function fetchContributions() {
      ...
      try {
        ...
        console.error(`Failed to load contributions for year ${selectedYear}:`, err);
      } finally {
          ...
          <div className="bg-bg2/25 backdrop-blur-lg border border-border/70 rounded-xl p-8 md:p-10 shadow-xl shadow-black/25 transition-all duration-300 relative overflow-hidden">
            {loadingContributions && !contributions ? (
              ...
              <div className="relative">
                {/* Premium translucent glass overlay during year fetch */}
                {isFetchingYear && (
                  <div className="absolute inset-0 bg-bg/40 backdrop-blur-[1px] flex items-center justify-center z-20 rounded-xl transition-all duration-300 animate-[fade-in_0.2s_ease-out] select-none">
                    <div className="flex flex-col items-center gap-2.5 bg-bg2/90 border border-border/60 p-4 rounded shadow-xl shadow-black/50">
                      <div className="w-4 h-4 border border-t-transparent border-seal animate-spin rounded-full" />
                      <span className="font-mono text-[9px] text-text3 tracking-widest uppercase">Fetching {selectedYear}...</span>
                    </div>
                  </div>
                )}
