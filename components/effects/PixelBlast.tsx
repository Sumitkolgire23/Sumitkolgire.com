...
export function PixelBlast({
  ...
  // Read feature flag state client-side
  useEffect(() => {
    setAnimationsEnabled(getFeatureFlags().scrollAnimations);
  }, []);
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      // Listen to events on window globally so clicks still pass through pointer-events: none elements
  ...
  // Set VisibilityObserver for page auto pause
  useEffect(() => {
    if (!animationsEnabled) return;
